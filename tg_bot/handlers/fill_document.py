from ..keyboards.inline import FillDocumentCallbackData
...
from ..tmp import CHAINS_OF_STATES, LEXICON, DIRECTORY_FOR_PHOTOS, DOWNLOAD_PHOTO, DIRECTORY_FOR_LATEX_FILES, WITH_FILL_FILE_MESSAGE, DIRECTORY_FOR_TEMPLATES
from ..tmp import update_photo_buffer, get_buffer_of_photos
...
@fsm_router.callback_query(FillDocumentCallbackData.filter(), StateFilter(default_state))
...
async def process_final_state_sent(message: types.Message, state: FSMContext, bot):
    ...
    await file_page_proceccing(message,
                               ...
                               document_name=user_data['document_name'], bot=bot)
